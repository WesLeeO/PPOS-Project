...
int utils_cat_first_sector(const struct unix_filesystem *u, uint16_t inr){
    ...
    if(res == ERR_NONE){
        ...
        if((f.i_node).i_mode & IFDIR){
            ...
            filev6_readblock(&f, tab);
           size_t i = 0;
           while(i < SECTOR_SIZE && tab[i] != '\0' && tab[i] != '\n'){
            pps_printf("%c", tab[i]);
            i++;
           }
            pps_printf("\n----\n");
...
int utils_print_shafile(const struct unix_filesystem *u, uint16_t inr){
    M_REQUIRE_NON_NULL(u);
    struct filev6 *f = malloc(sizeof(struct filev6));
    int err = filev6_open(u, inr, f);
    if(err == ERR_NONE){
        if (f->i_node.i_mode & IFDIR){
            pps_printf("SHA inode %d: DIR\n", inr);
            free(f);
            return ERR_NONE;
        }
        else{
