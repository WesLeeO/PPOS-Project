...
class Errors:
    ...
    def __init__(self, enum_filename: str, enum_name: str, error_filename: str, error_array_name: str, **kwargs):
        """
    ...
    def compare_exit_code(self, res, enum_name: str):
        ...
        """
        enum_ordinal = 0 if enum_name == "ERR_NONE" else self.get_enum_ordinal(enum_name)
        rc = self.match_signedness(res.rc, enum_ordinal)

        if rc != enum_ordinal:
            error_name = [name for name, ord in self.enum_mapping.items() if ord == rc] or 'Unknown error'
            raise Exception(f"Returned code {rc} ('{error_name}') != expected {enum_ordinal} ('{enum_name}'). Program output:\n{res.stdout}")
    ...
    def get_error_message(self, enum_name):
        ...
        """
        enum_ordinal = self.get_enum_ordinal(enum_name)
        return self.error_messages[enum_ordinal]
    ...
    def check_error_code_and_message(self, result, enum_name):
        ...
        assert len(lines) > 0, f"Expected stdout to contain '{expected}' but it was empty."
        ...
        assert expected in last_line, f"Expected last line of stdout to contain '{expected}' but was '{last}'"